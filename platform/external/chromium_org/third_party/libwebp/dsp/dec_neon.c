...
static void ApplyFilter4(
  ...
  *op0 = FlipSignBack(vqaddq_s8(p0, a2));  
  *oq0 = FlipSignBack(vqsubq_s8(q0, a1));  
  *op1 = FlipSignBack(vqaddq_s8(p1, a3));  
  *oq1 = FlipSignBack(vqsubq_s8(q1, a3));  
}
...
static void ApplyFilter6(
  ...
  const int16x8_t s1_lo = vmlal_s8(kCst63, kCst27, delta_lo);  
  const int16x8_t s1_hi = vmlal_s8(kCst63, kCst27, delta_hi);  
  const int16x8_t s2_lo = vmlal_s8(kCst63, kCst18, delta_lo);  
  const int16x8_t s2_hi = vmlal_s8(kCst63, kCst18, delta_hi);  
  const int16x8_t s3_lo = vmlal_s8(kCst63, kCst9, delta_lo);   
  const int16x8_t s3_hi = vmlal_s8(kCst63, kCst9, delta_hi);   
  ...
  *op0 = FlipSignBack(vqaddq_s8(p0, a1));  
  *oq0 = FlipSignBack(vqsubq_s8(q0, a1));  
  *oq1 = FlipSignBack(vqsubq_s8(q1, a2));  
  *op1 = FlipSignBack(vqaddq_s8(p1, a2));  
  *oq2 = FlipSignBack(vqsubq_s8(q2, a3));  
  *op2 = FlipSignBack(vqaddq_s8(p2, a3));  
}
