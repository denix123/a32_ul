...
static WEBP_INLINE void TransformColor(const VP8LMultipliers* const m,
                                       uint32_t* argb_data,
                                       int num_pixels) {
  const __m128i g_to_r = _mm_set1_epi32(m->green_to_red_);       
  ...
  for (i = 0; i + 4 <= num_pixels; i += 4) {
    const __m128i in = _mm_loadu_si128((__m128i*)&argb_data[i]);
    const __m128i alpha_green_mask = _mm_set1_epi32(0xff00ff00);  
...
static void ConvertBGRAToRGBA(const uint32_t* src,
  ...
  while (num_pixels >= 8) {
    ...
    const __m128i rg0 = _mm_unpacklo_epi8(rb0, ga0);   
    const __m128i ba0 = _mm_unpackhi_epi8(rb0, ga0);   
    const __m128i rgba0 = _mm_unpacklo_epi16(rg0, ba0);  
    const __m128i rgba4 = _mm_unpackhi_epi16(rg0, ba0);  
