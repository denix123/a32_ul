...
void helper_fprem1(CPUX86State *env)
{
    ...
    if (isinf(st0) || isnan(st0) || isnan(st1) || (st1 == 0.0)) {
        ST0 = double_to_floatx80(env, 0.0 / 0.0); 
        env->fpus &= (~0x4700); 
        return;
    }
...
void helper_fstenv(CPUX86State *env, target_ulong ptr, int data32)
{
    ...
    if (data32) {
        ...
        cpu_stl_data(env, ptr + 12, 0); 
        cpu_stl_data(env, ptr + 16, 0); 
        cpu_stl_data(env, ptr + 20, 0); 
...
void helper_fxsave(CPUX86State *env, target_ulong ptr, int data64)
{
...
#ifdef TARGET_X86_64
    if (data64) {
        cpu_stq_data(env, ptr + 0x08, 0); 
        cpu_stq_data(env, ptr + 0x10, 0); 
    } else
#endif
    {
        cpu_stl_data(env, ptr + 0x08, 0); 
        cpu_stl_data(env, ptr + 0x0c, 0); 
        cpu_stl_data(env, ptr + 0x10, 0); 
        cpu_stl_data(env, ptr + 0x14, 0); 
    }
    ...
    if (env->cr[4] & CR4_OSFXSR_MASK) {
        ...
        cpu_stl_data(env, ptr + 0x18, env->mxcsr); 
        cpu_stl_data(env, ptr + 0x1c, 0x0000ffff); 
