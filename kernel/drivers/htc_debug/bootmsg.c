...
static void erase_reboot_debug_flag (struct work_struct *work)
{
	...
	if (!strncmp(mem_reboot_debug->magic, MEM_BSP_REBOOT_DEBUG_MAGIC, strlen(MEM_BSP_REBOOT_DEBUG_MAGIC))){
		pr_info("%s Compare MAGIC OK, erase the reboot debug flag\n", __func__);
		memset(mem_reboot_debug->magic, 0, sizeof(struct msm_bsp_reboot_debug));
	}
}
