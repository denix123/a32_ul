...
#define FFS_ENABLED_TIMEOUT_MS 100
	if (board_get_usb_ats()) {
		if (0 == wait_event_interruptible_timeout(dev->wait_close_callback_q,
			!config->opened, msecs_to_jiffies(FFS_ENABLED_TIMEOUT_MS))) {
			pr_info("wait for adb close complete timeout (%d) ms \n",FFS_ENABLED_TIMEOUT_MS);
		}
		else
			pr_info("%s:adb close complete \n",__func__);
	}
...
static void functionfs_closed_callback(struct ffs_data *ffs)
{
	...
	if (board_get_usb_ats())
		wake_up_interruptible(&dev->wait_close_callback_q);
