		...
		if (func == get_string_array_value(p->functions,p->num_functions)) {
			...
			*vendor_id = p->vendor_id;
			for (j = 0;j < p->num_functions;j++) {
				...
				functions_name++;
				if (f == NULL) {
					/*++ 2014/04/15, USB Team,  PCN00001_FIX_KP_BY_CHECK_SPECIAL_ORDER_COMBINATION++*/
					printk(KERN_WARNING "[USB] %s : fail to get usb function %s\n",__func__,*functions_name);
					/*-- 2014/04/15, USB Team,  PCN00001_FIX_KP_BY_CHECK_SPECIAL_ORDER_COMBINATION--*/
					return -EINVAL; /* this case shouldn't happen*/
				}
...
*/
	...
	list_for_each_entry(f_holder, &conf->enabled_functions, enabled_list) {
		pr_debug("# %s\n", f_holder->f->name);
		/*++ 2015/5/12, USB Team, PCN00069 ++*/
		if(!strcmp(f_holder->f->name,"mtp") || !strcmp(f_holder->f->name,"rndis"))
			dev->cdev->gadget->streaming_enabled = true;
		/*++ 2015/5/12, USB Team, PCN00069 ++*/
	}
...
/*++ 2014/12/10, USB Team, PCN00055 ++*/
static ssize_t store_usb_function_switch(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	// add variable "carry" to avoid unknown stack corruption case,
	// if carry is not 0, stack corrupption happened.
