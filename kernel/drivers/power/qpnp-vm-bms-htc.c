...
struct qpnp_bms_chip {
	...
	struct delayed_work		write_emmc_work;
	struct wake_lock 		write_emmc_lock;
};
...
int pm8916_bms_store_battery_data_emmc(void)
{
	...
	if (delayed_work_pending(&the_chip->write_emmc_work)) {
		cancel_delayed_work_sync(&the_chip->write_emmc_work);
		wake_unlock(&the_chip->write_emmc_lock);
	}

	wake_lock(&the_chip->write_emmc_lock);
	schedule_delayed_work(&the_chip->write_emmc_work, 0);

	return 0;
}

static void qpnp_bms_write_emmc(struct work_struct *work)
{
	int rc;
	...
	if (the_chip->dt.store_batt_data_soc_thre > 0
		...
		rc = emmc_misc_write(BMS_STORE_MAGIC_NUM, BMS_STORE_MAGIC_OFFSET);
		if (!rc)
			write_fail = true;

		rc = emmc_misc_write(store_emmc.store_soc, BMS_STORE_SOC_OFFSET);
		if (!rc)
			write_fail = true;

		rc = emmc_misc_write(store_emmc.store_currtime_ms, BMS_STORE_CURRTIME_OFFSET);
		if (!rc)
			write_fail = true;
		...
		if (!write_fail) {
			pr_info("Stored soc=%d,currtime_ms=%lu,retry_times=%d\n",
				store_emmc.store_soc, store_emmc.store_currtime_ms, retry_times);
			retry_times = 0;
		} else {
			pr_info("Store fail. retry_times:%d\n", retry_times);
			if(retry_times < 3) {
				/* write fail, retry after 1 second */
				wake_lock(&the_chip->write_emmc_lock);
				schedule_delayed_work(&the_chip->write_emmc_work,
...
int pm8916_bms_consistence_check(void)
{
	...
	if (the_chip->dt.batt_stored_magic_num == BMS_STORE_MAGIC_NUM
			...
			/* For removable battery, we consider warm boot case only */
			if (the_chip->warm_reset || the_chip->dt.is_embeded_batt) {
	...
	pr_info("warm_reset:%d,batt_stored_magic_num:0x%X,store_batt_data_soc_thre:%d,batt_temp:%d,"
		...
		the_chip->dt.batt_stored_update_time,the_chip->dt.is_embeded_batt,consistent_flag);
...
int pm8916_bms_warmmax_by_batterydata(void) {
	...
	if (the_chip->batt_data->warmmax_mv != -EINVAL)
		return the_chip->batt_data->warmmax_mv;
	else
		return 0;
}
...
int pm8916_bms_coolmax_by_batterydata(void) {
	...
	if (the_chip->batt_data->coolmax_mv != -EINVAL)
		return the_chip->batt_data->coolmax_mv;
	else
		return 0;
}
bool pm8916_bms_batt_disabled_by_sw(void) {
	...
	if (the_chip->batt_data->batt_disabled_by_sw != -EINVAL){
		enable = the_chip->batt_data->batt_disabled_by_sw;
		pr_info("batt_disabled_by_sw %d\n", enable);
		return enable;
	}
	else {
		pr_info("batt_disabled_by_sw is not set\n");
		return false;
	}
}
/* htc interface -- */
...
static int parse_bms_dt_properties(struct qpnp_bms_chip *chip)
{
	...
	SPMI_PROP_READ_OPTIONAL(is_embeded_batt, "is-embeded-batt", rc);
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	/* htc: do not hold wake lock when low battery. */
	...
	INIT_DELAYED_WORK(&chip->write_emmc_work, qpnp_bms_write_emmc);

	wake_lock_init(&chip->write_emmc_lock, WAKE_LOCK_SUSPEND,
			"htc_battery_write_emmc");
