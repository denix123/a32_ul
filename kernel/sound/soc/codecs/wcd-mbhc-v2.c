...
{							\
	pr_info("%s: Acquiring BCL\n", __func__);	\
	mutex_lock(&mbhc->codec_resource_lock);		\
	pr_info("%s: Acquiring BCL done\n", __func__);	\
}

#define WCD_MBHC_RSC_UNLOCK(mbhc)			\
{							\
	pr_info("%s: Release BCL\n", __func__);	\
...
static void wcd_program_btn_threshold(const struct wcd_mbhc *mbhc, bool micbias)
{
	...
	for (i = 0; i <  btn_det->num_btn; i++) {
		...
		pr_info("%s: course: %d fine: %d reg_addr: %x reg_val: %x\n",
	...
	uint32_t *zr)
{
	...
	if (mbhc->skip_imped_detection){
		pr_info("%s: Skip imped detect RL %d ohm, RR %d ohm\n",
	...
	snd_soc_update_bits(codec,
			MSM8X16_WCD_A_ANALOG_MICB_2_EN,
			0x80, 0x00);
	pr_info("%s: Setup for impedance det\n", __func__);
	...
	pr_info("%s: Start performing impedance detection\n",
		 __func__);
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (!(jack_type & SND_JACK_LINEOUT)) {
			...
			pr_info("%s: Skip reporting JACK_LINEOUT", __func__);
		}
...
static irqreturn_t wcd_mbhc_hs_ins_irq(int irq, void *data)
{
	...
	pr_info("%s: detection_type %d, result2 %x\n", __func__,
...
static irqreturn_t wcd_mbhc_hs_rem_irq(int irq, void *data)
{
	...
	do {
		...
		result2 = (snd_soc_read(codec,
			MSM8X16_WCD_A_ANALOG_MBHC_ZDET_ELECT_RESULT));
		pr_info("%s: check result2 for fake removal: %x\n",
			 __func__, result2);
		if ((!(result2 & 0x01)) &&
...
irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	result1 = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MBHC_BTN_RESULT);
	pr_info("%s: Reporting btn press result1 =%d\n", __func__, result1);
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	snd_soc_write(codec, MSM8X16_WCD_A_ANALOG_MBHC_BTN3_CTL, 0x26);
